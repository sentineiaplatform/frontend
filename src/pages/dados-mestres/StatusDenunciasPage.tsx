...
function exportarCsvStatus(linhas: StatusDenunciaMock[]) {
  ...
  a.href = url
  a.download = `status-denuncias-${new Date().toISOString().slice(0, 10)}.csv`
  a.click()
  ...
  return (
    <div className="flex flex-col gap-4 md:gap-5">
        ...
        title={
          <span className="inline-flex items-center gap-2.5">
            <span className="bg-primary/8 text-primary inline-flex size-9 shrink-0 items-center justify-center rounded-lg">
              <CircleDot className="size-[1.15rem]" strokeWidth={1.75} aria-hidden />
            </span>
            <span>Status denúncias</span>
          </span>
        }
        description="Cadastro de status do fluxo de denúncias (dados de referência)."
      >
        <Tooltip>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              size="icon-sm"
              className="text-muted-foreground hover:text-foreground size-9"
              onClick={() => exportarCsvStatus(dadosFiltrados)}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  onClick={() => exportarCsvStatus(registrosSelecionados)}
                >
      ...
      <RegistrosListaPaginada
        className="mt-0"
        areaLabel="Tabela de status de denúncia"
        paginacao={{
